...
/* maiora list */
    ...
    INSERT INTO opera_maiora_temp
    (author_id, work_id, date_display, abbr, full, bibliography, comment, author_sort, work_sort, search, gq_work_id, gq_author_id, viaf_id, gnd_id, same_line)
        SELECT
            author.id,
            work.id,
            work.date_display,
    ...
    INSERT INTO opera_maiora_temp
    (author_id, date_display, abbr, full, comment, author_sort, work_sort, gq_author_id, viaf_id, gnd_id)
        SELECT author.id, author.date_display, IF(author.in_use != 1, CONCAT('[', author.abbr, ']'), author.abbr), author.full, author.txt_info, author.abbr_sort, "", author.gq_id, author.VIAF, author.GND
        FROM author
    ...
    (author_id, work_id, date_display, abbr, full, search, bibliography, comment, editions_id, editions_url, editions_label, gq_work_id, gq_author_id, viaf_id, gnd_id, same_line)
        SELECT author_id, work_id, date_display, abbr, full, search, bibliography, comment, editions_id, editions_url, editions_label, gq_work_id, gq_author_id, viaf_id, gnd_id, same_line
...
/* opera minora */
    CREATE OR REPLACE TABLE opera_minora_temp (id INT PRIMARY KEY AUTO_INCREMENT, author_id INT, work_id INT, date_display VARCHAR(200), citation TEXT, bibliography TEXT, search VARCHAR(300), editions_id JSON, editions_url JSON, editions_label JSON, gq_author_id INT, gq_work_id INT, viaf_id VARCHAR(30), gnd_id VARCHAR(15));

    INSERT INTO opera_minora_temp(author_id, work_id, date_display, citation, bibliography, search, gq_work_id, gq_author_id, viaf_id, gnd_id) SELECT author.id, work.id, work.date_display, CONCAT(IF(work.in_use = 0, "<del>", ""), REPLACE(work.opus, " <cit></cit> ( <cit_bib></cit_bib>)", ""), IF(work.in_use = 0, "</del>", ""), IF(work.reference IS NOT NULL AND work.reference != "", CONCAT("&nbsp;&nbsp;&nbsp;<i>v.</i> ", work.reference), "")), work.txt_info, work.ac_web, work.gq_id, author.gq_id, author.VIAF, author.GND FROM work LEFT JOIN author ON author.id = work.author_id WHERE is_maior = 0 ORDER By author.abbr_sort, work.abbr_sort;
    ...
    CREATE OR REPLACE TABLE opera_minora (id INT PRIMARY KEY AUTO_INCREMENT, author_id INT, work_id INT, date_display VARCHAR(200), citation TEXT, bibliography TEXT, search VARCHAR(300), editions_id JSON, editions_url JSON, editions_label JSON, gq_author_id INT, gq_work_id INT, viaf_id VARCHAR(30), gnd_id VARCHAR(15));
    INSERT INTO opera_minora
    (author_id, work_id, date_display, citation, bibliography, search, editions_id, editions_url, editions_label, gq_work_id, gq_author_id, viaf_id, gnd_id)
        SELECT author_id, work_id, date_display, citation, bibliography, search, editions_id, editions_url, editions_label, gq_work_id, gq_author_id, viaf_id, gnd_id
