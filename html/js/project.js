...
// -I- onLoad
// ****************************************************
function onProjectLoad(me){
    var detailView = me.ctn.querySelector("div.project_detail");
    var menuView = me.ctn.querySelector("div.article_container");
    argos.o.project.defaultGroupId = null;
    me._getJSON("/data/project_articles", function(rData){
        var keyWords = new KeyWordSetter();
        if(rData.length > 0){detailView.textContent = ""}
        // set max depth of article groups
        var maxDepth = {};
        for(var article of rData){
            var cDepth = (article.position.match(/\./g)||[]).length;
            var cGroup = article.position.substring(0,3);
        ...
        // add autocomplete
        let nOpusHidden = document.createElement("INPUT");
        nOpusHidden.type= "HIDDEN";
        nOpusHidden.id = "opus_hidden";
        me.ctn.appendChild(nOpusHidden);
        //display articles
        for(var article of rData){
            let cError = [];
        ...
        me._getJSON("/data/project_zettels", function(rData){
            argos.o.project.resultIds = [];
            for(let zettel of rData){
            ...
            // set selection etc.
            me.setSelection("main", "div.detail_zettel", true);
            me.setSelection("opus", "input.opus", false, function(){
            ...
            me.setSelection("article", "span.da_article", false, function(){
                saveArticleName(event.target.textContent, event.target.id);
            });
            me.setSelection("stelle", "span.stelle", false, function(){
                me._put("data/zettel/"+event.target.id, {"stellenangabe": event.target.textContent}, function(){me.refresh()});
            });
            me.setSelection("exportText", "span.exportText", false, function(){
