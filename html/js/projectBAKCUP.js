...
class Project extends Oculus{
    ...
    async load(){
        ...
        const articles = await arachne.article.bound([this.resId, "000"], [this.resId, "?"], "default", false);
        ...
        for(const article of articles){
            ...
            let zettels = await arachne.zettel_lnk.bound([article.id, "", 0, 0, 0], [article.id, "?", 9999, 9999, 9999, 9999], "zettel", false);
    ...
    editArticleStructure(){
        ...
        this.ctn.querySelectorAll(".editMode").forEach(function(e){e.classList.toggle("editModeOff")});
        this.ctn.querySelectorAll(".artBox").forEach(function(e){
            e.classList.toggle("editModeArticle");
            e.setAttribute("draggable", this.editMode);
        });
        this.ctn.querySelectorAll(".detail_zettel").forEach(function(e){
            e.setAttribute("draggable", !this.editMode);
        });
    }
}
